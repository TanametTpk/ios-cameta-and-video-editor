...
class StopMotion {
    ...
    func render(images:[UIImage] , completion: @escaping (URL)->Void){
        
        let settings = RenderSettings()
        let imageAnimator = ImageAnimator(renderSettings: settings , images: images)
        imageAnimator.render() {
            completion(settings.outputURL as URL)
        }
...
struct RenderSettings {
    ...
    var width: CGFloat = UIScreen.main.bounds.width//1280
    var height: CGFloat = UIScreen.main.bounds.height//720
    var fps: Int32 = 12   // 2 frames per second
...
//        let x = newSize.width < size.width ? (size.width - newSize.width) / 2 : 0
        ...
        context?.draw(image.cgImage!, in: CGRect(x: -newSize.height, y: y, width: newSize.height, height: newSize.width))
